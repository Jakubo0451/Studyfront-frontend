...
export default function Artifact({ 
  initialArtifactId, // Optional: Pre-select a specific artifact by ID
  onArtifactSelect, // Optional: Callback when an artifact is selected
  allowMultiple = false, // Optional: Allow selecting multiple artifacts
  mode = 'normal' // 'normal' or 'standalone' - standalone includes its own add/remove controls
}) {
    ...
    useEffect(() => {
      if (initialArtifactId) {
        // This would typically fetch the artifact details by ID from your backend
        // For now, we'll simulate it with a hardcoded name
        setSelectedArtifactName(`Artifact ${initialArtifactId}`);
    ...
    }, [initialArtifactId, filePreviewUrls]);
    ...
    const addArtifact = () => {
      ...
      setArtifacts([...artifacts, { id: newId, type: 'select' }]);
    };
    ...
    function selectArtifact(artifactId, artifactName) {
      return () => {
            ...
            {artifacts.map(artifact => (
              <div key={artifact.id} className={styles.artifactContainer}>
                ...
                <div className={styles.artifact}>
                  <SingleArtifact artifactType="select" id={artifact.id} />
                  {renderArtifactPopup()}
                </div>
              </div>
            ))}
    ...
    return (
      <div className={styles.artifact}>
        <SingleArtifact artifactType="select" />
        {renderArtifactPopup()}
      </div>
    );
    
    // Helper function to render the popup
    function renderArtifactPopup() {
      return (
              ...
              <div className={styles.selectArtifactMenu}>
                  ...
                  <div>
                      <div className={styles.uploadArtifact}>
                          <label htmlFor="fileUpload">Upload new artifact(s):</label>
                          <input 
                              type="file" 
