...
export default function StudyTakeComponent({ study, previewMode = false }) {
    ...
    if (typeof window !== "undefined" && !previewMode) {
      try {
        ...
        if (savedSession) {
          const parsedSession = JSON.parse(savedSession);
          if (
            parsedSession.responses &&
            Object.keys(parsedSession.responses).length > 0
          ) {
            console.log(
              "Restored responses from session:",
              parsedSession.responses
            );
  ...
  useEffect(() => {
    ...
    const initializeFingerprint = async () => {
      try {
        ...
        const fallbackId = `p_fallback_${Math.random()
          .toString(36)
  ...
  const checkParticipationStatus = async (studyId, visitorId) => {
    try {
      ...
      const response = await fetch(
        `${backendUrl}/api/responses/check-participation`,
        {
          method: "POST",
          headers: {
  ...
  const handleResponse = (questionId, responseFromChild) => {
    ...
    console.log(
      `PARENT StudyTakeComponent: Setting response for Q_ID ${questionId}:`,
      JSON.stringify(responseFromChild)
    );
    setResponses((prev) => ({
      ...prev,
  ...
  const handleNext = async () => {
    ...
    const currentQuestionDetails = questions.find(
    ...
    if (
      typeof responseForCurrentQuestion === "object" &&
      responseForCurrentQuestion !== null &&
      !Array.isArray(responseForCurrentQuestion)
    ) {
      isCurrentQuestionAnswered =
        Object.keys(responseForCurrentQuestion).length > 0 &&
      ...
      isCurrentQuestionAnswered =
        responseForCurrentQuestion !== null &&
        responseForCurrentQuestion !== undefined &&
  ...
  const handleSubmit = async () => {
    ...
    const unansweredQuestions = questions.filter((q) => {
      ...
      if (
        ...
        return !Object.values(response).some((val) =>
          Array.isArray(val)
            ? val.length > 0
            : val !== null && val !== "" && val !== undefined
        );
    ...
    if (unansweredQuestions.length > 0) {
      const unansweredTitles = unansweredQuestions
        .map(
          (q) =>
            q.data?.title ||
            `Question ${
              study.questions.findIndex((sq) => sq._id === q._id) + 1
            }`
        )
        .join(", ");
      alert(
        `Please answer all questions before submitting. Missing: ${unansweredTitles}`
      );
      return;
    }
    ...
    try {
      const responsesArray = Object.entries(responses).map(
        ([questionId, response]) => {
          // Find the question object from study.questions
  ...
  const renderAlreadyCompletedMessage = () => {
    return (
      <div className="bg-white rounded-lg shadow-md p-8 text-center">
        <h2 className="text-2xl font-bold mb-4 text-petrol-blue">
          You've Already Completed This Study
        </h2>
        <p className="mb-6 text-gray-600">
          Our records show that you have already submitted responses for this
          study. Each participant can only complete a study once.
        </p>
        <button
    ...
    return (
      <div className="mt-[1rem] w-[60%]">
        <img
          ...
          alt="Studyfront Logo"
        />
        <h1 className="text-4xl text-center font-bold mb-[3rem] text-petrol-blue">
          {study.title}
        </h1>

        {/* Description */}
        <div className="bg-sky-blue/20 p-4 rounded-md max-h-60 overflow-y-auto mb-4 border border-sky-blue">
          <h2 className="text-xl text-oxford-blue font-semibold mb-2">
            About this study
          </h2>
          <p className="text-oxford-blue whitespace-pre-line">
            {study.description}
          </p>
        </div>
        ...
        {study.hasTermsAndConditions && (
          <div className="mb-8">
            <p className="mt-[1.5rem] mb-[1rem] text-oxford-blue">
              This study requires you to accept Terms and Conditions before
              starting.
            </p>
            <div className="bg-sky-blue/20 p-4 rounded-md max-h-60 overflow-y-auto mb-4 border border-sky-blue">
              <h2 className="text-xl text-oxford-blue font-semibold mb-2">
      ...
      <div>
        ...
        {!hasStarted && !showDemographics ? (
          ...
          <div className="bg-white rounded-lg shadow-md p-8 text-center">
            ...
            <p className="mb-6">
              This was a preview - your responses were not submitted.
            </p>
          </div>
        ) : (
  ...
  return (
    <div className="w-[70%]">
      <h1 className="text-3xl mb-6 text-center text-petrol-blue">
        {study.title}
      </h1>
        ...
        <div>
          ...
          <div className="question-container">
            <QuestionRenderer
              ...
              onResponse={(response) =>
                handleResponse(currentQuestion._id, response)
              }
      ...
      <p className="text-right text-red-500 hidden" id="pleaseAnswer">
